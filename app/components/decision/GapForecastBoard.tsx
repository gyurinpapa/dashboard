...
function GapForecastBoardComponent({ snapshot }: Props) {
  ...
  const problemDefinitionText = useMemo(() => {
    const forecastText = formatMetricValue(
      snapshot.primaryMetric,
      snapshot.forecastValue,
    );

    const goalText = formatMetricValue(snapshot.primaryMetric, snapshot.goalValue);

    if (snapshot.gapValue <= 0) {
      if (snapshot.pacingStatus === "ahead") {
        return `현재 운영 목표 KPI는 ${metricName}이며, 현재 pace 기준 월말 예상치는 ${forecastText}입니다. 목표 ${goalText} 대비 초과 달성 가능성이 높으므로, 부족분 보전보다 초과 성과 유지와 품질 관리가 우선입니다.`;
      }

      return `현재 운영 목표 KPI는 ${metricName}이며, 현재 pace 기준 월말 예상치는 ${forecastText}입니다. 목표 ${goalText} 달성이 가능한 상태이며, 현재 흐름을 안정적으로 유지하는 것이 중요합니다.`;
    }

    if (lowerBetter) {
      return `현재 운영 목표 KPI는 ${metricName}이며, 현재 pace 기준 월말 예상치는 ${forecastText}입니다. 목표 ${goalText} 대비 초과 구간을 줄이기 위한 액션 우선순위 설정이 필요합니다.`;
    }

    return `현재 운영 목표 KPI는 ${metricName}이며, 현재 pace 기준 월말 예상치는 ${forecastText}입니다. 목표 대비 부족분을 메우기 위한 액션 우선순위 설정이 필요합니다.`;
  }, [
    snapshot.primaryMetric,
    snapshot.forecastValue,
    snapshot.goalValue,
  ...
  const operationInsightText = useMemo(() => {
    if (snapshot.gapValue <= 0) {
      if (snapshot.pacingStatus === "ahead") {
        return "현재 실적 페이스가 시간 진도보다 충분히 앞서 있으므로 목표 달성 자체보다, 현재 성과가 일시 급등인지 안정적 추세인지 확인하고 남은 기간 동안 효율 훼손 없이 초과 성과를 유지·확대할 구간을 선별하는 것이 중요합니다.";
      }

      return "현재 실적 페이스가 목표선과 유사하게 유지되고 있으므로, 변동성 관리와 안정적 운영을 통해 목표 달성을 확정짓는 것이 중요합니다.";
    }
