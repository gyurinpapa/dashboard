...
async function main(): Promise<void> {
  ...
  try {
    ...
    const requestKeywordScopePreserved =
      statsResult.keywordId ===
      keyword.id;

    const datesWithinRange =
      statsResult.records.every(
        (record) =>
          record.date >=
            input.dateFrom &&
    ...
    console.log(
      "stats request succeeded:",
      true,
    );
    console.log(
      "stats records returned:",
      statsResult.records.length,
    );
    console.log(
      "stats request keyword scope preserved:",
      requestKeywordScopePreserved,
    );
    console.log(
      "stats dates within range:",
      datesWithinRange,
    );
    console.log(
      "stats numeric fields valid:",
      numericFieldsValid,
    );
...
main().catch((error: unknown) => {
  if (
    ...
    console.error(
      "Naver stats verification failed:",
      error.code,
      error.status ?? "",
    );
