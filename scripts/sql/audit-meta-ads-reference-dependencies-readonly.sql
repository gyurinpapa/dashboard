-- Etrylue Performance: reference schema evidence only; NOT a migration.
-- Run the WHOLE file in the same project's SQL editor that supplied the baseline.
-- Export all seven result rows as JSON. Confirm the project ref separately;
...
WITH RECURSIVE
requested(name) AS (
  VALUES ('tenants'::text), ('workspaces'), ('advertisers'), ('report_types')
),
targets AS (
  SELECT r.name, c.oid, c.relkind, c.relowner, c.relacl,
...
table_evidence AS (
  SELECT '20_table'::text AS section, 'public.' || t.name AS object_name,
    jsonb_build_object(
      ...
      'role_access_before_rls', (SELECT jsonb_agg(jsonb_build_object(
        ...
        'schema_usage', CASE WHEN r.oid IS NOT NULL THEN has_schema_privilege(r.oid, 'public', 'USAGE') END,
...
result AS (
  SELECT '00_context'::text AS section, 'audit_context'::text AS object_name,
    jsonb_build_object(
      'audit_version', 'etrylue-meta-reference-dependencies-readonly-v1',
  ...
  SELECT '90_manifest', 'completeness', jsonb_build_object(
    'requested_tables', (SELECT jsonb_agg('public.' || name ORDER BY name) FROM requested),
    'tables_found', (SELECT count(*) FROM targets WHERE oid IS NOT NULL),
    'missing_tables', coalesce((SELECT jsonb_agg('public.' || name ORDER BY name) FROM targets WHERE oid IS NULL), '[]'::jsonb),
    'unsupported_relations', coalesce((SELECT jsonb_agg('public.' || name ORDER BY name) FROM targets
      WHERE oid IS NOT NULL AND (relkind <> 'r' OR relispartition)), '[]'::jsonb),
    'expected_result_rows', 7,
