/*
 * Etrylue Performance
 * Exact one-time Naver Search Ads recovery checkpoint repair
 *
 * Purpose:
 ...
 * - Candidate total rows are read dynamically from the job/checkpoint/live staging state.
 ...
 * - only candidate.error_detail and candidate.updated_at change
 * - no new job
 ...
 *
 * One-time behavior:
 * - The current candidate must match the exact reduced isolation shape.
 * - After repair that shape no longer matches, so a second execution fails.
...
declare
  ...
  v_expected_candidate_attempt_count constant bigint := 9;
  ...
  v_checkpoint jsonb;
  v_recovery jsonb;
  v_expected_reduced_recovery jsonb;
...
begin
  ...
  if not found then
    raise exception using
      errcode = 'P0002',
      message = 'RECOVERY_CHECKPOINT_REPAIR_CANDIDATE_NOT_FOUND';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_CANDIDATE_STATE_MISMATCH';
  end if;
  ...
  if v_authoritative_tail_rows < 0 then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_CANDIDATE_ROW_BOUNDARY_MISMATCH';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_ERROR_DETAIL_INVALID';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_ERROR_DETAIL_SHAPE_MISMATCH';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_CHECKPOINT_INVALID';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_CHECKPOINT_STATE_MISMATCH';
  end if;

  v_recovery :=
    v_checkpoint -> 'recovery';

  if v_recovery is null
     or jsonb_typeof(v_recovery) <> 'object'
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_RECOVERY_INVALID';
  end if;

  v_expected_reduced_recovery :=
    jsonb_build_object(
      'source_job_id', v_source_job_id,
      'confirmation_token', v_old_confirmation_token,
      'expected_current_ingestion_id', v_current_ingestion_id,
      'expected_published_ingestion_id', v_published_ingestion_id,
      'isolated', true
    );

  if v_recovery is distinct from v_expected_reduced_recovery then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_REDUCED_SHAPE_MISMATCH';
  end if;
  ...
  if not found then
    raise exception using
      errcode = 'P0002',
      message = 'RECOVERY_CHECKPOINT_REPAIR_SOURCE_JOB_NOT_FOUND';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_SOURCE_JOB_STATE_MISMATCH';
  end if;
  ...
  if not found then
    raise exception using
      errcode = 'P0002',
      message = 'RECOVERY_CHECKPOINT_REPAIR_REPORT_NOT_FOUND';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_REPORT_STATE_MISMATCH';
  end if;
  ...
  if v_active_job_count <> 0 then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_ACTIVE_JOB_EXISTS';
  end if;
  ...
  if v_required_unique_constraint_count <> 2 then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_STAGING_UNIQUE_CONTRACT_MISMATCH';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_SOURCE_STAGING_MISMATCH';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_CANDIDATE_STAGING_MISMATCH';
  end if;
  ...
  if v_candidate_prefix_mismatch_rows <> 0 then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_SOURCE_PREFIX_MISMATCH';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_REPORT_ROWS_MISMATCH';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_INGESTION_METADATA_MISMATCH';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_REPORT_ROW_SENTINEL_MISMATCH';
  end if;
  ...
  if not found then
    raise exception using
      errcode = '40001',
      message = 'RECOVERY_CHECKPOINT_REPAIR_UPDATE_NOT_APPLIED';
  end if;

  if (
    to_jsonb(v_candidate_after) - 'error_detail' - 'updated_at'
  ) is distinct from (
    to_jsonb(v_candidate) - 'error_detail' - 'updated_at'
  )
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_UPDATE_SCOPE_BREACHED';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_EXTERNAL_STATE_CHANGED';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_CANDIDATE_STAGING_CHANGED';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_SOURCE_STAGING_CHANGED';
  end if;
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'RECOVERY_CHECKPOINT_REPAIR_REPORT_SENTINEL_CHANGED';
  end if;

  raise notice 'recovery checkpoint repair passed: true';
  raise notice 'candidate job id: %', v_candidate_after.id;
  raise notice 'candidate attempt count unchanged: %',
