/*
 * Etrylue Performance
 * Read-only post-partial safety verification for exact Naver recovery candidate (attempt 11)
 *
 * SELECT only:
...
params as (
  select
    ...
    '431150b842a7a3297f8cfcb9d245182d37c4d774efad160a2c98d64b4338af6a'::text
...
checks as (
  select
    ...
    coalesce(
      ...
      false
    ) as candidate_updated_after_previous_attempt_ok,
...
select
  (
    ...
    and candidate_updated_after_previous_attempt_ok
  ...
  array_remove(
    array[
      ...
      case when not candidate_updated_after_previous_attempt_ok
        then 'candidate_updated_after_previous_attempt_ok' end,
  ...
  candidate_updated_after_previous_attempt_ok,
