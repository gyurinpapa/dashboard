// scripts/verify-media-sync-discovery-partial-resume-orchestration.ts
...
const EMPTY_CAMPAIGN_PAGE_SIZE =
  100;

const ACTIVE_CAMPAIGN_BASE_SEARCH_ID =
  "discovery-resume-campaign-page-2";
...
const DISCOVERY_PAGE_LIMIT_PER_RUN =
  1;
...
type FixtureCounters = {
  campaignPageRequests: number;
  adgroupPageRequests: number;
  keywordPageRequests: number;
...
function createEmptyCampaigns():
  NaverSearchAdsCampaignRecord[] {
  return Array.from(
    {
      length:
        EMPTY_CAMPAIGN_PAGE_SIZE,
    },
    (
      _,
      index,
      ...
      return {
        id:
          `discovery-empty-campaign-id-${sequence}`,

        name:
          `discovery-empty-campaign-${sequence}`,
...
function createActiveCampaign():
  ...
  return {
    id:
      "discovery-active-campaign-id",

    name:
      "discovery-active-campaign",
...
function createActiveAdgroup(
  campaignId: string,
): NaverSearchAdsAdgroupRecord {
  return {
    id:
      "discovery-active-adgroup-id",

    campaignId,

    name:
      "discovery-active-adgroup",
...
function createFixtureKeywords(
  ...
  return Array.from(
    ...
    (
      ...
      return {
        id:
          `discovery-keyword-id-${sequence}`,

        adgroupId,

        keyword:
          `discovery-keyword-${sequence}`,
...
function createListPage<T>(input: {
  records: T[];
...
function createFixtureDependencies(input: {
  ...
  activeCampaign: NaverSearchAdsCampaignRecord;
  activeAdgroup: NaverSearchAdsAdgroupRecord;
  keywords:
  ...
  return {
    fetchCampaignPage:
      async (
        ...
        >
      > => {
        input.counters.campaignPageRequests +=
          1;

        const baseSearchId =
          request.baseSearchId ?? null;

        if (baseSearchId === null) {
          return createListPage({
            records:
              [
                ...input.emptyCampaigns,
              ],

            baseSearchId,

            nextBaseSearchId:
              ACTIVE_CAMPAIGN_BASE_SEARCH_ID,
          });
        }
        ...
        throw new Error(
          "VERIFICATION_UNKNOWN_CAMPAIGN_PAGE_CURSOR",
        );
      },
    ...
    fetchAdgroupPage:
      async (
        ...
        >
      > => {
        input.counters.adgroupPageRequests +=
          1;
    ...
    fetchKeywordPage:
      async (
        ...
        >
      > => {
        input.counters.keywordPageRequests +=
          1;

        if (
          request.adgroupId !==
          input.activeAdgroup.id
        ) {
          throw new Error(
...
function getCheckpointCursor(
  ...
  if (!isPlainObject(cursor)) {
    throw new Error(
      "VERIFICATION_CHECKPOINT_CURSOR_MISSING",
    );
  }
...
async function assertPartialDidNotMutateReport(input: {
  fixture: VerificationFixture;
  reportStateBefore: ReportState;
}): Promise<void> {
  ...
  if (
    reportStateDuringPartial.currentIngestionId !==
      input.reportStateBefore.currentIngestionId ||
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!discoveryPartial) {
      throw new Error(
        "VERIFICATION_DISCOVERY_PARTIAL_RETURNED_NO_JOB",
      );
    }

    if (
      discoveryPartial.status !==
      "partial"
    ) {
      throw new Error(
        "VERIFICATION_DISCOVERY_RUN_DID_NOT_RETURN_PARTIAL",
      );
    }

    if (
      discoveryPartial.staging.collector
        .partialReason !==
      "max_keyword_discovery_pages_per_run_reached"
    ) {
      throw new Error(
        "VERIFICATION_DISCOVERY_PARTIAL_REASON_INVALID",
      );
    }

    if (
      counters.statsRequests !== 0 ||
      discoveryPartial.staging
        .runCanonicalRowCount !== 0 ||
      discoveryPartial.staging
        .canonicalRowCount !== 0 ||
      discoveryPartial.expectedRows !== 0
    ) {
      throw new Error(
        "VERIFICATION_DISCOVERY_PARTIAL_ENTERED_STATS_OR_ROWS",
      );
    }

    const discoveryJobState =
    ...
    if (
      discoveryJobState.status !== "pending" ||
      discoveryJobState.error !== null ||
      discoveryJobState.snapshot_ingestion_id !==
        null ||
      discoveryJobState.finished_at !== null ||
      discoveryJobState.raw_rows !== 0 ||
      discoveryJobState.normalized_rows !== 0 ||
      discoveryJobState.inserted_rows !== 0 ||
      discoveryJobState.failed_rows !== 0 ||
      discoveryCursor.campaignBaseSearchId !==
        ACTIVE_CAMPAIGN_BASE_SEARCH_ID
    ) {
      throw new Error(
        "VERIFICATION_DISCOVERY_CHECKPOINT_STATE_INVALID",
      );
    }

    await assertPartialDidNotMutateReport({
      fixture,
      reportStateBefore,
    });

    const statsPartialRuns:
      number[] = [];
    ...
    for (
      ...
      if (!orchestration) {
        throw new Error(
          "VERIFICATION_RESUME_RETURNED_NO_JOB_BEFORE_COMPLETION",
        );
      }
      ...
      if (
        ...
        statsPartialRuns.push(
          runIndex,
        );

        if (
          orchestration.staging.collector
            .partialReason !==
          "max_keyword_stats_per_run_reached"
        ) {
          throw new Error(
            "VERIFICATION_STATS_PARTIAL_REASON_INVALID",
          );
        }
        ...
        if (
          ...
          jobState.error !== null ||
          jobState.snapshot_ingestion_id !==
            null ||
          ...
          throw new Error(
            "VERIFICATION_STATS_PARTIAL_STATE_INVALID",
          );
        }
    ...
    if (
      ...
      statsPartialRuns.length < 2
    ) {
      throw new Error(
        "VERIFICATION_STATS_PARTIAL_RUN_COUNT_TOO_LOW",
      );
    }
    ...
    console.log(
      "discovery partial returned:",
      discoveryPartial.status ===
        "partial",
    );

    console.log(
      "discovery partial reason:",
      discoveryPartial.staging.collector
        .partialReason,
    );

    console.log(
      "discovery stats requests:",
      0,
    );
    ...
    console.log(
      "discovery returned to pending:",
      discoveryJobState.status ===
        "pending",
    );

    console.log(
      "stats partial runs:",
      statsPartialRuns.length,
    );
