...
type ClaimMeasurement = {
  claimNumber: number;
  attemptCount: number;
  status: "partial" | "cutover_guard_blocked";
...
function assertProductionCutoverConfirmationAbsent():
  void {
  if (
    readProductionCutoverConfirmationValue()
  ) {
    throw new Error(
      "VERIFICATION_PRODUCTION_CUTOVER_CONFIRMATION_MUST_BE_UNSET_FOR_GUARD",
    );
  }
}
    ...
    orchestrationDependencies: {
      ...
      activate:
        async (
          ...
          assertProductionCutoverConfirmationAbsent();
           ...
           * production cutover readiness와 confirmation-token 부재를
           * 모두 확인한 직후 guard blocker로 중단한다.
           */
          throw new Error(
...
async function main():
  ...
  let productionCutoverConfirmationToken:
    string | null =
      null;
  ...
  let finalConnectionStateUnchanged =
    false;
  ...
  console.log(
    "Naver Search Ads production cutover guard verification date range:",
  ...
  console.log(
    "actual activation RPC and finalization are blocked by production cutover guard:",
    true,
  );

  console.log(
    "production cutover confirmation env:",
    PRODUCTION_CUTOVER_CONFIRMATION_ENV,
  );

  console.log(
    "verified production cutover confirmation token absent before candidate:",
    readProductionCutoverConfirmationValue() ===
      "",
  );
  ...
  try {
    ...
    let expectedProductionCutoverGuardBlockObserved =
      false;
    ...
    for (
      ...
      try {
        ...
        assertExpectedProductionCutoverGuardBlock(
          error,
        );

        expectedProductionCutoverGuardBlockObserved =
          true;
        ...
        console.log(
          "real materialization completed and production cutover readiness verified before guard blocker:",
          JSON.stringify({
            claimNumber,
            attemptCount:
              claimedJob.attempt_count,
    ...
    if (!activationReadiness) {
      throw new Error(
        "VERIFICATION_ACTIVATION_READINESS_RESULT_MISSING",
      );
    }
    ...
    const connectionStateBeforeCleanup =
      await readConnectionLifecycleState(
        input.connectionId,
      );

    const connectionStateUnchangedBeforeCleanup =
      connectionStateBefore !==
        null &&
      connectionLifecycleStateMatches(
        connectionStateBefore,
        connectionStateBeforeCleanup,
      );
    ...
    const verificationPassed =
      ...
      connectionStateUnchangedBeforeCleanup &&
      readProductionCutoverConfirmationValue() ===
        "" &&
      productionCutoverConfirmationToken !==
        null &&
    ...
    console.log(
      "verified activation input scope matches materialized snapshot:",
      activationReadiness
        .activationInputScopeMatches,
    );

    console.log(
      "verified activation previous_ingestion_id matches current pointer:",
      activationReadiness
        .previousIngestionMatchesCurrentPointer,
    );

    console.log(
      "verified activation published pointer baseline is preserved:",
      activationReadiness
        .publishedPointerPreserved,
    );

    console.log(
      "verified activation candidate is not current snapshot:",
      activationReadiness
        .candidateIsNotCurrentSnapshot,
    );

    console.log(
      "verified activation candidate is not published snapshot:",
      activationReadiness
        .candidateIsNotPublishedSnapshot,
    );

    console.log(
      "verified activation candidate ingestion is successful and scoped:",
      activationReadiness
        .candidateIngestionValid,
    );

    console.log(
      "verified activation candidate row count matches expected:",
      activationReadiness
        .candidateRowCountMatches,
    );

    console.log(
      "verified activation candidate period matches job:",
      activationReadiness
        .candidatePeriodMatches,
    );

    console.log(
      "verified activation candidate totals match parity:",
      activationReadiness
        .candidateTotalsMatchParity,
    );

    console.log(
      "verified activation candidate completion fingerprint matches materialization:",
      activationReadiness
        .candidateCompletionFingerprintMatches,
    );

    console.log(
      "verified current snapshot comparison loaded:",
      activationReadiness
        .currentSnapshotComparisonLoaded,
    );

    console.log(
      "verified current snapshot descriptor:",
      JSON.stringify(
        activationReadiness
          .currentSnapshot,
      ),
    ...
    console.log(
      "verified candidate snapshot descriptor:",
      JSON.stringify(
        activationReadiness
          .candidateSnapshot,
      ),
    ...
    console.log(
      "verified activation readiness comparison:",
      JSON.stringify(
        activationReadiness
          .comparison,
      ),
    ...
    console.log(
      "verified activation readiness before RPC:",
      activationReadiness.ready,
    );
    ...
    console.log(
      "verified activation readiness blocker calls:",
      guards.activationCalls,
    );

    console.log(
      "verified production cutover guard blocked before activation RPC:",
      expectedProductionCutoverGuardBlockObserved,
    );

    console.log(
      "verified production confirmation token remained absent:",
      readProductionCutoverConfirmationValue() ===
        "",
    );

    console.log(
      "verified actual activation RPC calls:",
      guards.activationRpcCalls,
    );
    ...
    console.log(
      "verified operating connection state remains unchanged before cleanup:",
      connectionStateUnchangedBeforeCleanup,
    );
    ...
    console.log(
      "verified report_rows metadata total restored after cleanup:",
      databaseStateBefore
        .reportRowsMetadataCount ===
        databaseStateAfterCleanup
          .reportRowsMetadataCount,
    ...
    console.log(
      "verified temporary report_rows removed after cleanup:",
      cleanupCompleted,
    );
    ...
    console.log(
      "verified operating connection state remains unchanged after cleanup:",
      finalConnectionStateUnchanged,
    );

    console.log(
      "production cutover confirmation token for final cutover:",
      productionCutoverConfirmationToken,
    );
    ...
    console.log(
      "fixture is production cutover guard only:",
      true,
    );
    ...
    console.log(
      "final operating connection state unchanged:",
      finalConnectionStateUnchanged,
    );
    ...
    if (
      ...
      try {
        ...
        if (!finalConnectionStateUnchanged) {
          process.exitCode = 1;
        }
      } catch (finalConnectionStateError) {
        console.error(
...
main().catch(
  (error: unknown) => {
    if (
      ...
      console.error(
        "production cutover guard verification failed:",
        JSON.stringify(
          readSafeErrorChain(
            error,
          ),
