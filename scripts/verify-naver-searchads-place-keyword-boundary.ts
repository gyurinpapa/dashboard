// scripts/verify-naver-searchads-place-keyword-boundary.ts
//
// Pure DI fixture.
// - no real Naver API requests
// - no database connection
// - no database writes
// - verifies PLACE remains in the registered-keyword path
...
async function main(): Promise<void> {
    ...
    Partial<NaverAuthoritativeEntityStatsCollectorDependencies> = {
      ...
      fetchAdgroupPage:
        async () => {
          ...
          throw new Error(
            "PLACE must not enter authoritative adgroup discovery.",
          );
        },
      ...
      fetchAdPage:
        async () => {
          ...
          throw new Error(
            "PLACE must not enter authoritative ad discovery.",
          );
        },
      ...
      fetchEntityDailyStats:
        async () => {
          ...
          throw new Error(
            "PLACE must not request authoritative entity stats.",
          );
        },
  ...
  console.log(
    "PLACE campaign contract resolved: true",
  );

  console.log(
    "PLACE authoritative grain: keyword",
  );
...
main().catch(
  (
    ...
    console.error(
      "PLACE keyword-boundary verification failed:",
      error,
    );
