...
spec=importlib.util.spec_from_file_location('runner',Path(__file__).with_name('resume-meta-ads-db-checkpoint-case-test.py'))
...
class RunnerTests(unittest.TestCase):
    ...
    def test_embedded_payload_integrity(self):
        self.assertEqual(len(self.payload['files']),44)
    ...
    def test_offline_prepare_with_minimal_dependencies(self):
        ...
        with tempfile.TemporaryDirectory(prefix='meta-runner-unit-') as temp:
            ...
            outputs=list(root.glob('etrylue-meta-page-db-*/results/manifest.json'))
