// scripts/verify-exact-naver-production-recovery-authoritative-reisolation-contract.ts
//
// Static, database-free contract verification for the exact one-time
// Naver Search Ads authoritative partial re-isolation repair SQL.
...
const SQL_PATH = resolve(
  process.cwd(),
  "scripts/sql/reisolate-exact-naver-production-recovery-authoritative-partial.sql",
);
...
assert.ok(
  ...
  "The re-isolation SQL must begin with its safety contract comment.",
);

requirePattern(
  /\bbegin;[\s\S]*\bdo\s+\$repair\$[\s\S]*\$repair\$;[\s\S]*\bcommit;\s*$/i,
  "The re-isolation must remain inside one explicit transaction.",
);
...
requirePattern(
  /v_expected_candidate_updated_at\s+constant timestamptz\s*:=\s*'2026-07-21 20:36:36\.598013\+00';/i,
  "The exact post-failure candidate updated_at guard changed.",
);
requirePattern(
  /v_expected_candidate_started_at\s+constant timestamptz\s*:=\s*'2026-07-21 20:35:38\.750339\+00';/i,
  "The exact post-claim candidate started_at guard changed.",
);
requirePattern(
...
requirePattern(
  /v_expected_candidate_attempt_count\s+constant bigint\s*:=\s*10;/i,
  "The candidate attempt_count guard must remain exactly 10.",
);
requirePattern(
...
requirePattern(
  /v_candidate\.raw_rows\s+is\s+distinct\s+from\s+45614/i,
  "The one-time re-isolation must guard the exact observed 45,614 rows.",
);
requirePattern(
  /v_candidate\.raw_rows\s*<\s*v_source_boundary/i,
  "Candidate totals must remain structurally separate from the source prefix.",
);
requirePattern(
...
forbidPattern(
  /\binsert\s+into\b/i,
  "The re-isolation SQL must not INSERT any row.",
);
forbidPattern(
  /\bdelete\s+from\b/i,
  "The re-isolation SQL must not DELETE any row.",
);
forbidPattern(
  /\btruncate\b/i,
  "The re-isolation SQL must not TRUNCATE any table.",
);
forbidPattern(
  /\bupdate\s+public\.(?!media_sync_jobs\b)[a-z0-9_]+/i,
  "The re-isolation SQL must not UPDATE any table except media_sync_jobs.",
);
...
assert.equal(
  setClause,
  "status = 'cancelled', started_at = null, error_detail = v_repaired_error_detail, updated_at = v_repair_time",
  "Only candidate.status, started_at, error_detail, and updated_at may change.",
);

const whereClause = updateMatch[2];
for (const guard of [
  /job\.status\s*=\s*'processing'/i,
  /job\.attempt_count\s*=\s*v_expected_candidate_attempt_count/i,
  /job\.updated_at\s*=\s*v_expected_candidate_updated_at/i,
  /job\.started_at\s*=\s*v_expected_candidate_started_at/i,
  /job\.finished_at\s+is\s+null/i,
  /job\.error\s+is\s+null/i,
...
requirePattern(
  /v_recovery\s*:=\s*v_checkpoint\s*->\s*'recovery';[\s\S]*if\s+v_recovery\s+is\s+not\s+null\s+then/i,
  "The one-time re-isolation must require missing recovery metadata.",
);
requirePattern(
  /RECOVERY_REISOLATION_RECOVERY_ALREADY_PRESENT/,
  "A second re-isolation or pre-existing recovery metadata must be rejected.",
);

requirePattern(
  /v_repaired_recovery\s*:=\s*jsonb_build_object\([\s\S]*?'contract_version'[\s\S]*?'source_job_updated_at'[\s\S]*?'source_staging_rows'[\s\S]*?'source_identity_digest'[\s\S]*?'keyword_counts_derived_from_staging'[\s\S]*?'request_counts_reconstructed'[\s\S]*?'prepared_at'[\s\S]*?'confirmation_token'[\s\S]*?'isolated'[\s\S]*?\);/i,
  "The full recovery contract must be reconstructed from immutable facts.",
);
requirePattern(
  /jsonb_set\(\s*v_checkpoint,\s*'\{recovery\}',\s*v_repaired_recovery,\s*true\s*\)/i,
  "Missing recovery metadata must be inserted into the saved checkpoint.",
);
...
requirePattern(
  /if\s+v_active_job_count\s*<>\s*1[\s\S]*active_candidate\.id\s*=\s*v_candidate_id[\s\S]*active_candidate\.status\s*=\s*'processing'/i,
  "Exactly one active job must exist and it must be the exact processing candidate.",
);

for (const code of [
  "RECOVERY_REISOLATION_CANDIDATE_STATE_MISMATCH",
  "RECOVERY_REISOLATION_CANDIDATE_ROW_BOUNDARY_MISMATCH",
  "RECOVERY_REISOLATION_CHECKPOINT_STATE_MISMATCH",
  "RECOVERY_REISOLATION_SOURCE_JOB_STATE_MISMATCH",
  "RECOVERY_REISOLATION_REPORT_STATE_MISMATCH",
  "RECOVERY_REISOLATION_STAGING_UNIQUE_CONTRACT_MISMATCH",
  "RECOVERY_REISOLATION_SOURCE_STAGING_MISMATCH",
  "RECOVERY_REISOLATION_CANDIDATE_STAGING_MISMATCH",
  "RECOVERY_REISOLATION_SOURCE_PREFIX_MISMATCH",
  "RECOVERY_REISOLATION_REPORT_ROWS_MISMATCH",
  "RECOVERY_REISOLATION_INGESTION_METADATA_MISMATCH",
  "RECOVERY_REISOLATION_UPDATE_SCOPE_BREACHED",
  "RECOVERY_REISOLATION_EXTERNAL_STATE_CHANGED",
  "RECOVERY_REISOLATION_CANDIDATE_STAGING_CHANGED",
  "RECOVERY_REISOLATION_SOURCE_STAGING_CHANGED",
  "RECOVERY_REISOLATION_REPORT_SENTINEL_CHANGED",
] as const) {
...
console.log(
  "exact authoritative partial re-isolation contract verification passed:",
  true,
);
...
console.log("candidate columns allowed to change: status / started_at / error_detail / updated_at");
console.log("exact observed candidate rows guarded at 45,614:", true);
console.log("source prefix fixed to 44,514:", true);
console.log("saved authoritative tail preserved at 1,100:", true);
