...
const SECONDARY_PREVIOUS_INGESTION_ID =
  "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb";

const SECONDARY_SNAPSHOT_INGESTION_ID =
  "cccccccc-cccc-4ccc-8ccc-cccccccccccc";

const SECONDARY_PUBLISHED_INGESTION_ID =
...
async function main(): Promise<void> {
  ...
  const currentIngestionByReportId =
    new Map<string, string>([
      ...
      [
        SECONDARY_REPORT_ID,
        SECONDARY_PREVIOUS_INGESTION_ID,
      ],
    ]);
  ...
  const publishedIngestionByReportId =
    new Map<string, string>([
      ...
      [
        SECONDARY_REPORT_ID,
        SECONDARY_PUBLISHED_INGESTION_ID,
      ],
    ]);
      ...
      async (
        ...
        if (
          ...
          assert.equal(
            materializationCount,
            1,
            "The primary report must materialize first.",
          );

          assert.equal(
            input.job.snapshot_ingestion_id,
            null,
            "The primary job snapshot mirror was already populated before primary materialization.",
          );
        ...
        assert.equal(
          targetReportId,
          SECONDARY_REPORT_ID,
          "An unexpected fanout report was materialized.",
        );

        assert.equal(
          materializationCount,
          2,
          "The secondary report must materialize after the primary report.",
        );

        assert.equal(
          input.job.snapshot_ingestion_id,
          SNAPSHOT_INGESTION_ID,
          "Secondary materialization changed or lost the primary compatibility snapshot mirror.",
        );
        ...
        return {
          job:
            input.job,
          snapshotIngestionId:
            SECONDARY_SNAPSHOT_INGESTION_ID,
          rowCount:
            RECONCILED_ROWS,
          stagingFingerprint:
            "b".repeat(
    ...
    loadProjectionAuthority:
      async (
        ...
        assert.equal(
          materializationCount,
          2,
          "Projection authority loaded before every report was materialized.",
        );
    ...
    activate:
      async (
        ...
        assert.equal(
          projectionAuthorityLoadCount,
          2,
          "Activation started before every projection authority was loaded.",
        );
        ...
        if (
          ...
          assert.equal(
            activationCount,
            1,
            "The secondary report must activate before the primary report.",
          );
          ...
          assert.equal(
            ...
            PREVIOUS_INGESTION_ID,
            "Primary current pointer moved during secondary activation.",
          );
        ...
        assert.equal(
          activationCount,
          2,
          "The primary report must activate last.",
        );
        ...
        assert.equal(
          ...
          SECONDARY_SNAPSHOT_INGESTION_ID,
          "Primary activation started before secondary activation completed.",
        );
    ...
    finalize:
      async (
        ...
        assert.equal(
          activationCount,
          2,
          "Execution finalized before both report projections were activated.",
        );
        ...
        assert.equal(
          ...
          SECONDARY_PUBLISHED_INGESTION_ID,
          "Secondary published_ingestion_id changed before finalization.",
        );
  ...
  assert.deepEqual(
    ...
    [
      ...
      `materialization:${SECONDARY_REPORT_ID}`,
      "targets:2",
      `projection:${INITIAL_JOB.report_id}`,
      `projection:${SECONDARY_REPORT_ID}`,
      `activation:${SECONDARY_REPORT_ID}`,
  ...
  assert.equal(
    ...
    SECONDARY_PUBLISHED_INGESTION_ID,
    "Secondary published_ingestion_id changed during combined worker orchestration.",
  );
  ...
  console.log(
    "verified completed lifecycle order: reconciliation / summary / targets / primary materialization / secondary materialization / target revalidation / projection authorities / secondary activation / primary activation / target revalidation / finalization",
  );

  console.log(
    "verified materialization calls after reconciled combined completion: 2",
  );

  console.log(
    "verified activation calls after all materializations: 2",
  );

  console.log(
    "verified finalization calls after both activations: 1",
  );

  console.log(
    "verified independent primary and secondary current_ingestion_id transitions: true",
  );

  console.log(
    "verified both published_ingestion_id values remain unchanged: true",
  );
  ...
  console.log(
    "verified projection authority loads: 2",
  );
  ...
  console.log(
    "verified secondary activation precedes primary activation: true",
  );
  ...
  console.log(
    "MACRO4_B2_SYNTHETIC_DI_FULL_FANOUT_PASS=true",
  );
}
