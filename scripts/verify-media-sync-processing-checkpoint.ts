...
} from "../src/lib/media-sync/media-sync-processing-checkpoint-repository";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    checkpointJob.failed_rows ===
        0 &&
    checkpointJob.snapshot_ingestion_id ===
        claimedJob.snapshot_ingestion_id &&
    checkpointJob.finished_at ===
        claimedJob.finished_at &&
    checkpointJob.error ===
        null &&
    checkpointJob.error_detail !==
        null &&
    typeof checkpointJob
        .error_detail
        .processing_checkpoint ===
        "object";

    console.log(
    "processing checkpoint saved:",
    checkpointSaved,
    );
    ...
    exactRetryJob.raw_rows ===
        checkpointJob.raw_rows &&
    exactRetryJob.normalized_rows ===
        checkpointJob.normalized_rows &&
    exactRetryJob.inserted_rows ===
        checkpointJob.inserted_rows &&
    exactRetryJob.status ===
        PROCESSING_STATUS;

    console.log(
    "exact checkpoint retry accepted:",
    exactRetryAccepted,
    );
    ...
    const jobCheckpointOnlyChanged =
    jobStateAfter.status ===
        PROCESSING_STATUS &&
    jobStateAfter.progress >=
        jobStateBefore.progress &&
    jobStateAfter.progress <=
        99 &&
    jobStateAfter.rawRows ===
        expectedRows &&
    jobStateAfter.normalizedRows ===
        expectedRows &&
    jobStateAfter.insertedRows ===
        expectedRows &&
    jobStateAfter.failedRows ===
        0 &&
    jobStateAfter.snapshotIngestionId ===
        jobStateBefore.snapshotIngestionId;

    console.log(
    "only processing checkpoint fields changed:",
    jobCheckpointOnlyChanged,
    );
