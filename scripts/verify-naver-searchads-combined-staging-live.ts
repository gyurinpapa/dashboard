...
type ClaimMeasurement = {
  claimNumber: number;
  attemptCount: number;
  status: "partial" | "materialization_blocked";
...
async function main():
  ...
  console.log(
    "combined live staging verification date range:",
  ...
  try {
    /*
     * DB read-only safety gate.
     * 하나라도 불일치하면 credential 복호화, Naver API 호출, temporary write 전에 중단한다.
     */
    const reportScope =
    ...
    const snapshotIngestionIdRemainsNull =
      finalJob
        .snapshot_ingestion_id ===
      null;
    ...
    const verificationPassed =
      ...
      guards.finalizationCalls ===
        0 &&
      snapshotIngestionIdRemainsNull &&
    ...
    console.log(
      "verified all row fingerprints are present:",
      storedRowsValidation
        .allFingerprintsPresent,
    );
    ...
    console.log(
      "verified Search Ads impressions:",
      storedRowsValidation
        .actualTotals
        .impressions,
    );
    ...
    console.log(
      "verified materialization blocker calls:",
      guards.materializationCalls,
    );

    console.log(
      "verified activation calls:",
      guards.activationCalls,
    );
    ...
    console.log(
      "verified snapshot_ingestion_id remains null:",
      snapshotIngestionIdRemainsNull,
    );

    console.log(
      "verified current_ingestion_id remains unchanged:",
      databaseStateBefore
        .currentIngestionId ===
        databaseStateAfterCleanup
          .currentIngestionId,
    ...
    console.log(
      "verified published_ingestion_id remains unchanged:",
      databaseStateBefore
        .publishedIngestionId ===
        databaseStateAfterCleanup
          .publishedIngestionId,
    ...
    console.log(
      "verified report_rows count remains unchanged:",
      databaseStateBefore
        .reportRowsCount ===
        databaseStateAfterCleanup
          .reportRowsCount,
    ...
    console.log(
      "verified report_ingestions count remains unchanged:",
      databaseStateBefore
        .reportIngestionsCount ===
        databaseStateAfterCleanup
          .reportIngestionsCount,
    ...
    console.log(
      "verified temporary staging and job cleanup completed:",
      cleanupCompleted,
    );
...
main().catch(
  (error: unknown) => {
    if (
      ...
      console.error(
        "combined live staging verification failed:",
        JSON.stringify(
          readSafeErrorChain(
            error,
          ),
