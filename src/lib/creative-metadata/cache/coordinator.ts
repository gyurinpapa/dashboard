...
export type RefreshStage='input'|'authorize'|'context'|'keys'|'pre_claim_revalidation'|'cache_claim'|'provider_collection'|'post_fetch_revalidation'|'publication'|'cache_finish'|'final_revalidation';
export type CachedResult=Readonly<{status:'disabled'|'ready'|'partial'|'busy'|'cooldown'|'budget'|'rejected'|'no_targets';
 problem:ServerProblem|null;failureStage:RefreshStage|null;entries:readonly Metadata[];counts:Counts;cacheHits:number;pendingTargets:number;hasMore:boolean;retryAt:number|null;
...
export async function runCachedServerMetadata(input:ServerInput,ports:Ports,store:CacheStore):Promise<CachedResult>{
 ...
 const result=(status:CachedResult['status'],entries:readonly Metadata[]=[],problem:ServerProblem|null=null,retryAt:number|null=null):CachedResult=>{
  ...
  return Object.freeze({status:status==='ready'&&expired?'partial':status,problem,failureStage:status==='rejected'?stage:null,entries:Object.freeze(valid),counts,cacheHits,
