...
const GOOGLE_ADS_SEARCH_AD_ROW_LEVEL =
  "creative" as const;

export const GOOGLE_ADS_SEARCH_AD_ROW_LEVEL_REASON =
  "google_ads_search_ad_daily_stats" as const;

export type GoogleAdsSearchAdCanonicalRowErrorCode =
  | "INVALID_INPUT"
  | "SCOPE_MISMATCH"
  | "INVALID_STATS_RECORD"
  | "DUPLICATE_DATE";

export class GoogleAdsSearchAdCanonicalRowError
  extends Error {
  readonly code:
    GoogleAdsSearchAdCanonicalRowErrorCode;
  ...
  constructor(
    ...
    this.name =
      "GoogleAdsSearchAdCanonicalRowError";
...
export type GoogleAdsSearchAdCanonicalCampaign =
  Readonly<{
    id: string;
    name: string;
  }>;

export type GoogleAdsSearchAdCanonicalAdGroup =
  Readonly<{
    id: string;
    campaignId: string;
    name: string;
...
export type GoogleAdsSearchAdCanonicalAd =
  Readonly<{
    id: string;
    adGroupId: string;
  }>;

export type GoogleAdsSearchAdDailyStatsRecord =
  Readonly<{
    date: string;
    adId: string;
    impressions: number;
...
export type ConvertGoogleAdsSearchAdDailyStatsToCanonicalRowsInput =
  Readonly<{
    ...
    records:
      readonly GoogleAdsSearchAdDailyStatsRecord[];
  }>;
...
function normalizeRequiredString(
  ...
  if (typeof value !== "string") {
    throw new GoogleAdsSearchAdCanonicalRowError(
      "INVALID_INPUT",
...
function normalizeStatsRecord(
  ...
  const typedRecord =
    record as GoogleAdsSearchAdDailyStatsRecord;
...
export function convertGoogleAdsSearchAdDailyStatsToCanonicalRows(
  input:
    ConvertGoogleAdsSearchAdDailyStatsToCanonicalRowsInput,
  ...
  if (
    ...
    throw new GoogleAdsSearchAdCanonicalRowError(
      "INVALID_INPUT",
      "Google Ads Search ad canonical row conversion input is required.",
    );
  }
    ...
    input.records.map(
      (
        ...
        if (
          ...
          throw new GoogleAdsSearchAdCanonicalRowError(
            "DUPLICATE_DATE",
            "The Google Ads Search ad stats input contains more than one row for the same ad and date.",
          );
        }
        ...
        return {
          ...
          row_level:
            GOOGLE_ADS_SEARCH_AD_ROW_LEVEL,
          data_level:
            GOOGLE_ADS_SEARCH_AD_ROW_LEVEL,
          row_level_reason:
