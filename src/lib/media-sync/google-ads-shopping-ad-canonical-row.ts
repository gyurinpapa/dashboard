...
const GOOGLE_ADS_SHOPPING_AD_ROW_LEVEL =
  "creative" as const;

export const GOOGLE_ADS_SHOPPING_AD_ROW_LEVEL_REASON =
  "google_ads_shopping_ad_daily_stats" as const;

export type GoogleAdsShoppingAdCanonicalRowErrorCode =
  | "INVALID_INPUT"
  | "SCOPE_MISMATCH"
  | "INVALID_STATS_RECORD"
  | "DUPLICATE_DATE";

export class GoogleAdsShoppingAdCanonicalRowError
  extends Error {
  readonly code:
    GoogleAdsShoppingAdCanonicalRowErrorCode;
  ...
  constructor(
    ...
    this.name =
      "GoogleAdsShoppingAdCanonicalRowError";
...
export type GoogleAdsShoppingAdCanonicalCampaign =
  Readonly<{
    id: string;
    name: string;
  }>;

export type GoogleAdsShoppingAdCanonicalAdGroup =
  Readonly<{
    id: string;
    campaignId: string;
    name: string;
...
export type GoogleAdsShoppingAdCanonicalAd =
  Readonly<{
    id: string;
    adGroupId: string;
  }>;

export type GoogleAdsShoppingAdDailyStatsRecord =
  Readonly<{
    date: string;
    adId: string;
    impressions: number;
...
export type ConvertGoogleAdsShoppingAdDailyStatsToCanonicalRowsInput =
  Readonly<{
    ...
    records:
      readonly GoogleAdsShoppingAdDailyStatsRecord[];
  }>;
...
function normalizeRequiredString(
  ...
  if (typeof value !== "string") {
    throw new GoogleAdsShoppingAdCanonicalRowError(
      "INVALID_INPUT",
...
function normalizeStatsRecord(
  ...
  const typedRecord =
    record as GoogleAdsShoppingAdDailyStatsRecord;
...
export function convertGoogleAdsShoppingAdDailyStatsToCanonicalRows(
  input:
    ConvertGoogleAdsShoppingAdDailyStatsToCanonicalRowsInput,
  ...
  if (
    ...
    throw new GoogleAdsShoppingAdCanonicalRowError(
      "INVALID_INPUT",
      "Google Ads Shopping ad canonical row conversion input is required.",
    );
  }
    ...
    input.records.map(
      (
        ...
        if (
          ...
          throw new GoogleAdsShoppingAdCanonicalRowError(
            "DUPLICATE_DATE",
            "The Google Ads Shopping ad stats input contains more than one row for the same ad and date.",
          );
        }
        ...
        return {
          ...
          row_level:
            GOOGLE_ADS_SHOPPING_AD_ROW_LEVEL,
          data_level:
            GOOGLE_ADS_SHOPPING_AD_ROW_LEVEL,
          row_level_reason:
