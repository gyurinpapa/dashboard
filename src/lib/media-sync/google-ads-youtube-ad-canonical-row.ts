...
const GOOGLE_ADS_YOUTUBE_AD_ROW_LEVEL =
  "creative" as const;

export const GOOGLE_ADS_YOUTUBE_AD_ROW_LEVEL_REASON =
  "google_ads_youtube_ad_daily_stats" as const;

export type GoogleAdsYoutubeAdCanonicalRowErrorCode =
  | "INVALID_INPUT"
  | "SCOPE_MISMATCH"
  | "INVALID_STATS_RECORD"
  | "DUPLICATE_DATE";

export class GoogleAdsYoutubeAdCanonicalRowError
  extends Error {
  readonly code:
    GoogleAdsYoutubeAdCanonicalRowErrorCode;
  ...
  constructor(
    ...
    this.name =
      "GoogleAdsYoutubeAdCanonicalRowError";
...
export type GoogleAdsYoutubeAdCanonicalCampaign =
  Readonly<{
    id: string;
    name: string;
  }>;

export type GoogleAdsYoutubeAdCanonicalAdGroup =
  Readonly<{
    id: string;
    campaignId: string;
    name: string;
...
export type GoogleAdsYoutubeAdCanonicalAd =
  Readonly<{
    id: string;
    adGroupId: string;
  }>;

export type GoogleAdsYoutubeAdDailyStatsRecord =
  Readonly<{
    date: string;
    adId: string;
    impressions: number;
...
export type ConvertGoogleAdsYoutubeAdDailyStatsToCanonicalRowsInput =
  Readonly<{
    ...
    records:
      readonly GoogleAdsYoutubeAdDailyStatsRecord[];
  }>;
...
function normalizeRequiredString(
  ...
  if (typeof value !== "string") {
    throw new GoogleAdsYoutubeAdCanonicalRowError(
      "INVALID_INPUT",
...
function normalizeStatsRecord(
  ...
  const typedRecord =
    record as GoogleAdsYoutubeAdDailyStatsRecord;
...
export function convertGoogleAdsYoutubeAdDailyStatsToCanonicalRows(
  input:
    ConvertGoogleAdsYoutubeAdDailyStatsToCanonicalRowsInput,
  ...
  if (
    ...
    throw new GoogleAdsYoutubeAdCanonicalRowError(
      "INVALID_INPUT",
      "Google Ads YouTube ad canonical row conversion input is required.",
    );
  }
    ...
    input.records.map(
      (
        ...
        if (
          ...
          throw new GoogleAdsYoutubeAdCanonicalRowError(
            "DUPLICATE_DATE",
            "The Google Ads YouTube ad stats input contains more than one row for the same ad and date.",
          );
        }
        ...
        return {
          ...
          row_level:
            GOOGLE_ADS_YOUTUBE_AD_ROW_LEVEL,
          data_level:
            GOOGLE_ADS_YOUTUBE_AD_ROW_LEVEL,
          row_level_reason:
