...
const GOOGLE_ADS_DISPLAY_AD_ROW_LEVEL =
  "creative" as const;

export const GOOGLE_ADS_DISPLAY_AD_ROW_LEVEL_REASON =
  "google_ads_display_ad_daily_stats" as const;

export type GoogleAdsDisplayAdCanonicalRowErrorCode =
  | "INVALID_INPUT"
  | "SCOPE_MISMATCH"
  | "INVALID_STATS_RECORD"
  | "DUPLICATE_DATE";

export class GoogleAdsDisplayAdCanonicalRowError
  extends Error {
  readonly code:
    GoogleAdsDisplayAdCanonicalRowErrorCode;
...
export type GoogleAdsDisplayAdCanonicalCampaign =
  Readonly<{
    id: string;
    name: string;
  }>;

export type GoogleAdsDisplayAdCanonicalAdGroup =
  Readonly<{
    id: string;
    campaignId: string;
    name: string;
...
export type GoogleAdsDisplayAdCanonicalAd =
  Readonly<{
    id: string;
    adGroupId: string;
  }>;

export type GoogleAdsDisplayAdDailyStatsRecord =
  Readonly<{
    date: string;
    adId: string;
    impressions: number;
...
export type ConvertGoogleAdsDisplayAdDailyStatsToCanonicalRowsInput =
  Readonly<{
    ...
    records:
      readonly GoogleAdsDisplayAdDailyStatsRecord[];
  }>;
...
function normalizeRequiredString(
  ...
  if (typeof value !== "string") {
    throw new GoogleAdsDisplayAdCanonicalRowError(
      "INVALID_INPUT",
...
function normalizeStatsRecord(
  ...
  const typedRecord =
    record as GoogleAdsDisplayAdDailyStatsRecord;
...
export function convertGoogleAdsDisplayAdDailyStatsToCanonicalRows(
  input:
    ConvertGoogleAdsDisplayAdDailyStatsToCanonicalRowsInput,
  ...
  if (
    ...
    throw new GoogleAdsDisplayAdCanonicalRowError(
      "INVALID_INPUT",
      "Google Ads Display ad canonical row conversion input is required.",
    );
  }
    ...
    input.records.map(
      (
        ...
        if (
          ...
          throw new GoogleAdsDisplayAdCanonicalRowError(
            "DUPLICATE_DATE",
            "The Google Ads Display ad stats input contains more than one row for the same ad and date.",
          );
        }
        ...
        return {
          ...
          row_level:
            GOOGLE_ADS_DISPLAY_AD_ROW_LEVEL,
          data_level:
            GOOGLE_ADS_DISPLAY_AD_ROW_LEVEL,
          row_level_reason:
